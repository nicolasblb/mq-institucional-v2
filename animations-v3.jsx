...
function Stage({
  ...
  // Embedded mode: the stage is a piece of someone else's layout, not the
  // page. It drops the playback bar, the dark backdrop and the canvas
  // drop-shadow, and stops auto-scaling — the host sizes the 1:1 box and
  // scales it with CSS, so nothing competes for svg.style.transform.
  embed = false,
  // Bumping resetSignal seeks the clock back to 0 and resumes playback.
  // This is the ONLY deterministic restart: the playhead is React state
  // here, so poking localStorage or remounting from outside races this.
  resetSignal = 0,
  children,
}) {
  ...
  if (typeof embed === 'string') embed = embed !== 'false';
  ...
  // Restart from the top, on the stage's own time path
  const resetRef = React.useRef(resetSignal);
  React.useEffect(() => {
  ...
  // Auto-scale to fit viewport (the host owns scale in embedded mode)
  React.useEffect(() => {
    if (embed || !stageRef.current) return;
      ...
      <div style={{
        ...
        <svg
          ...
          style={{
            ...
            flexShrink: 0,
            boxShadow: embed ? 'none' : '0 20px 60px rgba(0,0,0,0.4)',
            display: 'block',
          }}
        >
  ...
  return (
    <React.Fragment>
      <Stage width={width} height={height} duration={derived.total} background={bg}
             ...
             embed={props.embed} resetSignal={props.resetSignal}
